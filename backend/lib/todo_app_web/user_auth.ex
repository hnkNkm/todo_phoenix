defmodule TodoAppWeb.UserAuth do
  ...
  @doc """
  Plug that requires the user to have a password set.
  Used for routes that need a fully authenticated user.
  """
  def require_password_set(conn, _opts) do
    user = conn.assigns.current_scope && conn.assigns.current_scope.user
    
    if user && !user.hashed_password do
      conn
      |> put_flash(:info, "アカウントを保護するためにパスワードを設定してください。")
      |> redirect(to: ~p"/users/setup-password")
      |> halt()
    else
      conn
    end
  ...
  @doc """
  Fetches the user from the API token in the Authorization header.
  """
  def fetch_api_user(conn, _opts) do
    with ["Bearer " <> token] <- get_req_header(conn, "authorization"),
         {user, _inserted_at} when not is_nil(user) <- Accounts.get_user_by_session_token(token) do
